...
% optional argument to define the initial water depth
%hLi = 10;
if exist('hLi')
    hL = 'hL=filling20';
else
...
load(['./glacier_model/output/ELA1500/RCP8.5/slope' num2str(slope) '/' climate '/s0=0.75/output.' yearlabel '.mat'])


% compute basin geometry for each time step

if exist('hLi')
    basin_geometry = LakeWithIce_with_GC(sol,a,pL,basin,hLi); % returns initial water level and volume, depends on if there is ice in the basin
else
...
RunInfo.Qin_dim                = 20;  % Input to the lake m^3 s^-^1
RunInfo.InitialLakeDepthDim    = basin_geometry.hLi;  % initial lake level
...
if exist('hLi')
    RunInfo.InitialSGuess      = 0.1;  % in m^2
else
...
RunInfo.MRdim                  = 10^-5; %input into channel along length
...
mkdir(['outburst_flood_model/FullModel/ELA1500/RCP8.5/slope' num2str(slope) '/' climate '/s0=' num2str(s0) '/' basin_shape '/' basin '/' hL ]);           
save(['outburst_flood_model/FullModel/ELA1500/RCP8.5/slope' num2str(slope) '/' climate '/s0=' num2str(s0) '/' basin_shape '/' basin '/' hL '/FullModelt_years=' num2str(t_years(i),'%03.f') '___.mat'],'output','modelParams');
